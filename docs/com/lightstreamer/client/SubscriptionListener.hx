...
interface SubscriptionListener {
 ...
 /**
   ...
   * through `Subscription.removeListener(SubscriptionListener)`. This is the last event to be fired on the listener.
   * 
   ...
   * through `Subscription.addListener(SubscriptionListener)`. This is the first event to be fired on the listener.
   *
   ...
   * Subscription is performed multiple times through `LightstreamerClient.unsubscribe(Subscription)` and 
   ...
   * fired an `this.onUnsubscription` event is eventually fired. <BR> 
   ...
   * Note that, in order to perform a new subscription attempt, `LightstreamerClient.unsubscribe(Subscription)`
   * and `LightstreamerClient.subscribe(Subscription)` should be issued again, even if no change to the Subscription 
   ...
   * `LightstreamerClient.subscribe(Subscription)`. This can also happen multiple times in case of automatic 
   * recovery after a connection restart. <BR>
   * After this notification no more events can be received until a new `this.onSubscription` event. <BR> 
   * Note that two consecutive calls to this method are not possible, as before a second onUnsubscription event 
   * is fired an `this.onSubscription` event is eventually fired. <BR> 
   * If the involved Subscription has a two-level behavior enabled
   * (see `Subscription.setCommandSecondLevelFields(String[])` and `Subscription.setCommandSecondLevelFieldSchema(String)`)
