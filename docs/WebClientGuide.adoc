= Web Client Guide
Target API: Lightstreamer Web Client (Unified) v9.0.0
2023-01-23
:doctype: book
// Settings
...
https://github.com/Lightstreamer/Lightstreamer-lib-client-haxe[Github project].
...
A ready-made implementation of such interface is distributed with the library. 
The configuration of this logging facility should be different between development and
production environments.

The suggested approach is to use a console logger configured at INFO
level during development and at ERROR level
or no logger at all on production deployments.
...
messages produced by such logger (for instance, look for the Developer
Tools). 
