...
interface ClientListener {
   ...
   * through `LightstreamerClient.removeListener(ClientListener)`. This is the last event to be fired on the listener.
   ...
   * through `LightstreamerClient.addListener(ClientListener)`. This is the first event to be fired on the listener.
   ...
   * In both cases, the `this.onStatusChange` event handler has already been invoked
   ...
   * 
   * @see `this.onStatusChange(String)`
   ...
   * <ul>
   * <li>After issuing connect() when the current status is "DISCONNECTED&ast;", the client will switch to "CONNECTING" 
   ...
   * <li>On the other hand, after issuing connect when the status is already "CONNECTED:&ast;" a switch to "CONNECTING"
   ...
   * After that, the `this.onServerError` event handler will be invoked.</li>
   * </ul>
   * Possible special cases are the following:
   * <ul>
   * <li>In case of Server unavailability during streaming, the status may switch from "CONNECTED:&ast;-STREAMING" 
   * to "STALLED" (see `ConnectionOptions.setStalledTimeout(long)`). If the unavailability ceases, the status 
   * will switch back to "CONNECTED:&ast;-STREAMING"; otherwise, if the unavailability persists 
   * (see `ConnectionOptions.setReconnectTimeout(long)`), the status will switch to "DISCONNECTED:TRYING-RECOVERY"
   * and eventually to "CONNECTED:&ast;-STREAMING".</li>
   * <li>In case the connection or the whole session is forcibly closed by the Server, the status may switch 
   * from "CONNECTED:&ast;-STREAMING" or "CONNECTED:&ast;-POLLING" directly to "DISCONNECTED". After that, 
   * the `this.onServerError` event handler will be invoked.</li>
   * <li>Depending on the setting in `ConnectionOptions.setSlowingEnabled(boolean)`, in case of slow update processing, 
   ...
   * <li>If the status is "CONNECTED:&ast;-POLLING" and any problem during an intermediate poll occurs, the status may 
   * switch to "CONNECTING" and eventually to "CONNECTED:&ast;-POLLING". The same may hold for the "CONNECTED:&ast;-STREAMING" case, 
   * when a rebind is needed.</li>
   * <li>In case a forced transport was set through `ConnectionOptions.setForcedTransport(String)`, only the 
   * related final status or statuses are possible.</li>
   * <li>In case of connection problems, the status may switch from any value
   * to "DISCONNECTED:WILL-RETRY" (see `ConnectionOptions.setRetryDelay(long)`),
   ...
   * CONNECTED:&ast;-STREAMING status or will switch to "DISCONNECTED:WILL-RETRY" / "DISCONNECTED:TRYING-RECOVERY".</li>
